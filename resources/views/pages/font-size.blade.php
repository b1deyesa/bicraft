<x-layout.property category="Typography" title="Font Size" :tokens="$tokens">
    
    {{-- Apply --}}
    @foreach ($tokens['values'] as $index => $token)
        <div class="apply__item">
            <h5 class="item__title">{{ Str::of($token['key'])->replace('-', ' ') }}</h5>
            <div class="item__box">
                <div class="box__display">
                    <span class="{{ $tokens['prefix'] }}-{{ $token['key'] }}">Bicraft is where creativity meets craft in digital form.</span>
                </div>
                <div class="box__code">
